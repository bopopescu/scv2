    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if os.path.isfile(app.config['UPLOAD_FOLDER'] + itemtype_name + "/" + filename):
                ...
                db.session.commit()
                return render_template('pages/item.html', add_res=add_res, user_note=user_note, add=add, image_link=image_link, typeslist=res_all_itemtypes, myitem=myItemObject, myparticipants=myItemPartcipants, myNbReviews=myNbReviews)
    else:
        return redirect('/')
